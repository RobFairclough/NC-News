...
const sendAllArticles = (req, res, next) => {
    ...
    connection('articles')
        .select('articles.username AS author', 'articles.title', 'articles.article_id', 'articles.votes', 'articles.created_at', 'topic', 'users.avatar_url')
...
const sendArticleById = (req, res, next) => {
    ...
    connection('articles')
        .select('articles.article_id', 'articles.username AS author', 'title', 'articles.votes', 'articles.body', 'articles.created_at', 'topic')
        .leftJoin('comments', 'comments.article_id', 'articles.article_id')
        ...
        if (article) {
            ...
            next({ msg: `404, no article with ID ${article_id}` });
    })
        .catch(next);
};
...
const deleteArticle = (req, res, next) => {
    ...
    connection('articles')
        ...
        .then((response) => {
        if (response === 0)
            next({ status: 404, msg: 'no articles exist to delete with that id' });
        else
