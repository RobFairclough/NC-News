...
describe('/api', () => {
    ...
    describe('/:topic/articles', () => {
      it('GET request should respond status 200 and an array of article objects for a given topic, containing all required properties including an accurate comment count', () => request
        ...
        .then(({ body }) => {
          ...
          expect(body.articles[0]).to.have.all.keys('author', 'title', 'article_id', 'votes', 'created_at', 'comment_count', 'topic', 'avatar_url');
      ...
      it('GET request should ignore invalid queries', () => request
        .get('/api/topics/cats/articles?limit=theyredogs&sortby=dogbreed&p=pagefour&order=forwards')
        .expect(200)
        .then(({ body }) => {
          expect(body.articles.length).to.equal(1);
      ...
      it('POST request should return 400 if the username is not in the db', () => request
        .post('/api/topics/cats/articles')
        .send({
          title: 'Why its not a bad thing that cats leave dead birds and rodents in your house, and fun DIY recipes to make use of them!',
          username: 'notACat',
      ...
      it('unauthorised POST request should return status 401', () => request
        ...
        .send({
          ...
          username: 'rogersop',
          body: 'reasons why authentication is for squares and why i will be posting an article regardless',
        }));
      it('invalid request methods should return status 405', () => {
        const invalidMethods = ['put', 'patch', 'delete'];
  ...
  describe('/articles', () => {
    it('GET request should return status 200 and respond with an array of article objects, each object having properties author, title, article_id, body, votes, comment_count, created_at and topic', () => request
      ...
      .expect(200)
      .then(({ body }) => {
        expect(body.articles[0]).to.have.all.keys('author', 'title', 'article_id', 'votes', 'comment_count', 'created_at', 'topic', 'avatar_url');
      }));
  ...
  describe('/users', () => {
    ...
    it('POST request should respond status 201 and return the new user object', () => request
      ...
      .send({
        username: 'rob',
        password: 'password',
        avatar_url: 'https://www.gettyimages.co.uk/gi-resources/images/CreativeLandingPage/HP_Sept_24_2018/CR3_GettyImages-159018836.jpg',
        name: 'rob',
      })
      .expect(201)
      .then(({ body }) => {
